...
public:
  ...
  template <typename> friend class detail::AwaitableFrame;

  template <typename> friend class detail::AwaitableIOFrame;
  ...
  /// @brief Should be used carefully! Only for `get_return_object` in promise
  /// type.
...
template <class FrameType> struct Awaitable<void, FrameType> {
public:
  using promise_type = FrameType;

  template <typename> friend class detail::AwaitableFrame;

  template <typename> friend class AwaitableIOFrame;
...
template <class Value, class FrameType>
typename Awaitable<Value, FrameType>::promise_type *
Awaitable<Value, FrameType>::get_promise() {
  return this->frame_;
}
} // namespace usub::uvent::task
