...
namespace usub::uvent::sync
{
    ...
    class AsyncEvent
    {
        ...
        static bool try_claim(WaitNode* n) noexcept
        {
            ...
            return n->st.compare_exchange_strong(exp, NodeState::Claimed, std::memory_order_acq_rel,
    ...
    public:
        ...
        struct WaitAwaiter
        {
            ...
            bool await_ready() noexcept
            {
                ...
                {
                    bool expected = true;
                    if (self->set_.compare_exchange_strong(expected, false, std::memory_order_acquire,
                                                           std::memory_order_relaxed))
                        return true;
                    return false;
                }
