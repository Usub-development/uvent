...
#include "uvent/system/Defines.h" // TODO: Probably should move stuff out of Defines.h (force inline, TRAP and pther stuff)
...
inline constexpr uint64_t FLAGS_MASK =
    (CLOSED_MASK | BUSY_MASK | READING_MASK | WRITING_MASK | DISCONNECTED_MASK);
...
static_assert((TIMEOUT_EPOCH_MASK & DISCONNECTED_MASK) == 0,
              "epoch overlaps DISCONNECTED");
static_assert((TIMEOUT_EPOCH_MASK & BUSY_MASK) == 0, "epoch overlaps flags");
static_assert((TIMEOUT_EPOCH_MASK & COUNT_MASK) == 0,
              "epoch overlaps refcount");

template <class Derived> class RefCounted {
public:
  ...
  bool try_add_ref() noexcept {
    ...
    for (;;) {
      ...
      if (st.compare_exchange_weak(s, ns, std::memory_order_acquire,
...
protected:
  ...
  static constexpr bool is_closed(uint64_t s) noexcept {
    return (s & CLOSED_MASK) != 0;
  }
...
private:
#ifndef UVENT_ENABLE_REUSEADDR
  std::atomic<uint64_t> &state() noexcept {
