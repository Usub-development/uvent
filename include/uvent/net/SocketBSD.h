...
namespace usub::uvent::net
{
    ...
    // BSD/macOS only: accept() + fcntl(O_NONBLOCK | FD_CLOEXEC)
    template <Proto p, Role r>
    ...
    // BSD/macOS async_read -> DynamicBuffer
    template <Proto p, Role r>
    ...
    // BSD/macOS async_read -> raw buffer
    template <Proto p, Role r>
    task::Awaitable<ssize_t, uvent::detail::AwaitableIOFrame<ssize_t>>
        ...
        requires((p == Proto::TCP && r == Role::ACTIVE) || (p == Proto::UDP))
    {
        ...
        else
        {
            ...
            while (total_written < static_cast<ssize_t>(sz))
            {
                ...
                {
#if UVENT_DEBUG
                    spdlog::info("async_write: EAGAIN, waiting for write-ready: fd={}",
                                 this->header_->fd);
#endif
        ...
        requires((p == Proto::TCP && r == Role::ACTIVE) || (p == Proto::UDP))
    {
        ...
        if (sendRes != static_cast<size_t>(-1)) return std::move(this->receive(chunkSize, maxSize));
        return std::unexpected(usub::utils::errors::SendError::InvalidSocketFd);
    }

    // async_sendfile: только macOS и *BSD, без linux-ветки
    template <Proto p, Role r>
    ...
    // sync sendfile: только macOS и *BSD
    template <Proto p, Role r>
    ...
    {
        if (this->header_->fd < 0) return static_cast<size_t>(-1);
