...
namespace usub::uvent::net
{
    ...
    Socket<p, r>::async_accept() requires (p == Proto::TCP && r == Role::PASSIVE)
    {
        for (;;)
        {
            ...
            int cfd;
#ifdef OS_LINUX
            cfd = ::accept4(this->header_->fd,
                            reinterpret_cast<sockaddr*>(&ss), &sl,
                            SOCK_NONBLOCK | SOCK_CLOEXEC);
#else
        cfd = ::accept(this->header_->fd,
                       reinterpret_cast<sockaddr*>(&ss), &sl);
        if (cfd >= 0) {
            // NB: проверяем ошибки fcntl
            int flags = fcntl(cfd, F_GETFL, 0);
            if (flags == -1 || fcntl(cfd, F_SETFL, flags | O_NONBLOCK) == -1) { ::close(cfd); errno = EAGAIN; cfd = -1; }
            else {
                int fdflags = fcntl(cfd, F_GETFD, 0);
                if (fdflags == -1 || fcntl(cfd, F_SETFD, fdflags | FD_CLOEXEC) == -1) { ::close(cfd); errno = EAGAIN; cfd = -1; }
            }
        }
#endif
