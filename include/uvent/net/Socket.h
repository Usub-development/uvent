...
namespace usub::uvent::net
{
        ...
        requires(p == Proto::TCP && r == Role::PASSIVE)
    {
        for (;;)
        {
            ...
            int cfd;
#ifdef OS_LINUX
            cfd = ::accept4(this->header_->fd, reinterpret_cast<sockaddr*>(&ss), &sl,
                            SOCK_NONBLOCK | SOCK_CLOEXEC);
#else
            cfd = ::accept(this->header_->fd, reinterpret_cast<sockaddr*>(&ss), &sl);
            if (cfd >= 0) {
                // NB: проверяем ошибки fcntl
    ...
    // kqueue/bsd-ветка: одно ожидание + вычитать до EAGAIN
    co_await detail::AwaiterRead{this->header_};
    ...
    template <Proto p, Role r>
    task::Awaitable<ssize_t, uvent::detail::AwaitableIOFrame<ssize_t>> Socket<p, r>::async_write(
        ...
        requires((p == Proto::TCP && r == Role::ACTIVE) || (p == Proto::UDP))
    {
        ...
        else
        {
            ...
            {
                ssize_t res = ::send(this->header_->fd, buf + total_written,
                ...
                {
#if UVENT_DEBUG
                    spdlog::info("async_write: EAGAIN, waiting for EPOLLOUT: fd={}",
                                 this->header_->fd);
#endif
        ...
        requires(p == Proto::TCP && r == Role::ACTIVE)
    {
        ...
        // Если уже подключились (ret == 0) — можно не ждать события,
        // но если хочешь унифицированный путь, можно всё равно ждать Writable.
        system::this_thread::detail::pl->addEvent(this->header_, core::OperationType::ALL);
        ...
        // либо проверяешь флаг, либо делаешь getsockopt(SO_ERROR)
        ...
        requires(p == Proto::TCP && r == Role::ACTIVE)
    {
        ...
        addrinfo hints{}, *res = nullptr;
        hints.ai_family = (this->ipv == utils::net::IPV::IPV4) ? AF_INET : AF_INET6;
        hints.ai_socktype = SOCK_STREAM; // p всегда TCP по requires
        hints.ai_protocol = 0;
            ...
            for (;;)
            {
                ...
                if constexpr (p == Proto::TCP)
                {
                    res = ::send(this->header_->fd,
                                 ...
                                 sz - static_cast<size_t>(total_written),
                                 MSG_DONTWAIT);
                }
                else
                {
    ...
    template <Proto p, Role r>
    std::expected<std::string, usub::utils::errors::SendError> Socket<p, r>::send(uint8_t* buf,
        size_t sz,
        size_t chunkSize,
        size_t maxSize)
        requires((p == Proto::TCP && r == Role::ACTIVE) || (p == Proto::UDP))
    ...
    template <Proto p, Role r>
    task::Awaitable<ssize_t, uvent::detail::AwaitableIOFrame<ssize_t>> Socket<p, r>::async_sendfile(
...
#elif defined(__FreeBSD__) || defined(__DragonFly__) || defined(__OpenBSD__) || defined(__NetBSD__)
        // *BSD: int sendfile(in_fd, out_fd, off_t offset, nbytes, sf_hdtr*, off_t* sbytes, int
        // flags)
        off_t off = offset ? *offset : 0;
        off_t sbytes = 0;
        int rc = ::sendfile(in_fd, this->header_->fd, off,
