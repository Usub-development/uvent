...
namespace usub::uvent::net {
    ...
    template <Proto p, Role r>
    task::Awaitable<ssize_t, uvent::detail::AwaitableIOFrame<ssize_t>> Socket<p, r>::async_read(
        utils::DynamicBuffer& buffer, size_t max_read_size)
        requires((p == Proto::TCP && r == Role::ACTIVE) || (p == Proto::UDP))
    {
#if UVENT_DEBUG
    ...
    template <Proto p, Role r>
    task::Awaitable<ssize_t, uvent::detail::AwaitableIOFrame<ssize_t>> Socket<p, r>::async_read(
        uint8_t* dst, size_t max_read_size)
        requires((p == Proto::TCP && r == Role::ACTIVE) || (p == Proto::UDP))
    {
#if UVENT_DEBUG
        ...
        if constexpr (p == Proto::UDP) {
            for (;;) {
                ssize_t res = ::recvfrom(this->header_->fd, dst, max_read_size, MSG_DONTWAIT,
                                         nullptr, nullptr);
                if (res > 0) {
#ifndef UVENT_ENABLE_REUSEADDR
