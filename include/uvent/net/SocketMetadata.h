...
struct alignas(32) SocketHeader {
  ...
  UVENT_ALWAYS_INLINE void decrease_ref() noexcept {
  ...
  UVENT_ALWAYS_INLINE void close_for_new_refs() noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  UVENT_ALWAYS_INLINE bool try_mark_busy() noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  UVENT_ALWAYS_INLINE void clear_busy() noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  [[nodiscard]] UVENT_ALWAYS_INLINE bool is_busy_now() const noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  UVENT_ALWAYS_INLINE bool try_mark_reading() noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
    uint64_t s = this->state.load(std::memory_order_relaxed);
    for (;;) {
  ...
  UVENT_ALWAYS_INLINE void clear_reading() noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  UVENT_ALWAYS_INLINE bool is_reading_now() const noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  UVENT_ALWAYS_INLINE bool try_mark_writing() noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
    uint64_t s = this->state.load(std::memory_order_relaxed);
    for (;;) {
  ...
  UVENT_ALWAYS_INLINE void clear_writing() noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  UVENT_ALWAYS_INLINE bool is_writing_now() const noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  UVENT_ALWAYS_INLINE void mark_disconnected() noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  [[nodiscard]] UVENT_ALWAYS_INLINE bool is_disconnected_now() const noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  UVENT_ALWAYS_INLINE uint64_t timeout_epoch_snapshot() noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  UVENT_ALWAYS_INLINE uint64_t timeout_epoch_load() noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
    return (this->state.load(std::memory_order_acquire) & TIMEOUT_EPOCH_MASK);
#else
  ...
  UVENT_ALWAYS_INLINE void timeout_epoch_bump() noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  UVENT_ALWAYS_INLINE bool timeout_epoch_changed(uint64_t snap) noexcept {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
    return (this->state.load(std::memory_order_acquire) & TIMEOUT_EPOCH_MASK) !=
           snap;
  ...
  [[nodiscard]] UVENT_ALWAYS_INLINE bool
  is_done_client_coroutine_with_timeout() const {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  [[nodiscard]] UVENT_ALWAYS_INLINE uint64_t get_counter() const {
    using namespace usub::utils::sync::refc;
#ifndef UVENT_ENABLE_REUSEADDR
  ...
  [[nodiscard]] UVENT_ALWAYS_INLINE bool is_tcp() const {
    return (this->socket_info & static_cast<uint8_t>(Proto::TCP)) != 0;
  }

  [[nodiscard]] UVENT_ALWAYS_INLINE bool is_passive() const {
