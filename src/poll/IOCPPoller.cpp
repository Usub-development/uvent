...
namespace usub::uvent::core {
    ...
    IocpOp IOCPPoller::decode_op_(OVERLAPPED* ov) noexcept {
        ...
        if (tag == static_cast<uintptr_t>(IocpOp::WriteKick)) return IocpOp::WriteKick;

        if (!ov) return IocpOp::Unknown;
        auto he = reinterpret_cast<uintptr_t>(ov->hEvent);
        if (he == static_cast<uintptr_t>(IocpOp::Recv0)) return IocpOp::Recv0;
        if (he == static_cast<uintptr_t>(IocpOp::WriteKick)) return IocpOp::WriteKick;
    ...
    void IOCPPoller::addEvent(net::SocketHeader *header, OperationType initialState) {
        (void)CreateIoCompletionPort(reinterpret_cast<HANDLE>(static_cast<uintptr_t>(header->fd)),
                                     iocp_, reinterpret_cast<ULONG_PTR>(header), 0);
    ...
    void IOCPPoller::updateEvent(net::SocketHeader *header, OperationType initialState) {
        if (initialState == READ || initialState == ALL)
            arm_recv0_(header);
        if (initialState == WRITE || initialState == ALL)
            kick_write_(header);
    ...
    void IOCPPoller::arm_recv0_(net::SocketHeader* h) {
        if (h->iocp_read.posted) return;
        h->iocp_read.posted = true;

        ZeroMemory(&h->iocp_read.ov, sizeof(h->iocp_read.ov));
        h->iocp_read.ov.hEvent = reinterpret_cast<HANDLE>(static_cast<uintptr_t>(IocpOp::Recv0));
        ...
        DWORD flags = 0;
        DWORD recvd = 0;
        int rc = WSARecv((SOCKET)h->fd, &h->iocp_read.buf, 1, &recvd, &flags, &h->iocp_read.ov, nullptr);
        if (rc == SOCKET_ERROR) {
    ...
    void IOCPPoller::kick_write_(net::SocketHeader* h) {
        ...
        PostQueuedCompletionStatus(iocp_, 0,
                                   ...
                                   reinterpret_cast<OVERLAPPED*>(static_cast<uintptr_t>(IocpOp::WriteKick)));
    }
    ...
    bool IOCPPoller::poll(int timeout) {
...
#endif
        ULONG n = 0;
        BOOL ok = GetQueuedCompletionStatusEx(iocp_,
                                              entries_.data(),
                                              static_cast<ULONG>(entries_.size()),
                                              &n,
                                              to_wait_(timeout),
                                              FALSE);
        if (!ok) {
        ...
        for (ULONG i = 0; i < n; ++i) {
            ...
            if (op == IocpOp::Recv0) {
                ...
                if (e.dwNumberOfBytesTransferred == 0) {
                    ...
                    if (r == 0) { sock->mark_disconnected(); hup = true; }
                }
        ...
        if (n == entries_.size()) entries_.resize(entries_.size() << 1);
