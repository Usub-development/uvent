...
#endif
#else
thread_local std::unique_ptr<utils::TimerWheel> wh =
    std::make_unique<utils::TimerWheel>();
...
#else
thread_local std::unique_ptr<queue::single_thread::Queue<net::SocketHeader *>>
    q_sh = std::make_unique<queue::single_thread::Queue<net::SocketHeader *>>();
#endif
#ifndef UVENT_ENABLE_REUSEADDR
