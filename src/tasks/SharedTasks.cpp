...
namespace usub::uvent::task
{
    ...
    {
        this->detachedTasks = std::make_unique<queue::concurrent::SegmentedMPMCQueue<std::coroutine_handle<>>>();
    }

    void SharedTasks::enqueue(std::coroutine_handle<>& task) { this->detachedTasks->enqueue(task); }
